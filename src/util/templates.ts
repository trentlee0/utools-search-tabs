...
export interface NoneTemplate extends Feature {
  action: (enter?: {code: string; type: string; payload: any}) => void
}
    ...
    target.templates[item.code] = {
      ...
      args: {
        enter: (action, callbackSetList) => {
          if (
            item.onlyEnterOnce &&
            Object.entries(target.dynamicListStates[i]).length
          ) {
        ...
        search: (action, searchWord, callbackSetList) => {
          searchWord = searchWord.toLowerCase()
          callbackSetList(
            target.dynamicListStates[i].filter(
              ({title, description}) =>
                title.toLowerCase().includes(searchWord) ||
                (item.isSearchDesc &&
                  description.toLowerCase().includes(searchWord))
